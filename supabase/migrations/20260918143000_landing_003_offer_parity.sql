-- LANDING-003: iki tam sayfa tasarımını aynı teklif, ana mesaj ve kayıt
-- hedefiyle karşılaştırır. LANDING-001 ve LANDING-002 kayıtları tarihsel kalır.
alter table public.analytics_events
  drop constraint if exists analytics_events_experiment_id_check;

alter table public.analytics_events
  add constraint analytics_events_experiment_id_check
    check (experiment_id in ('', 'landing-001', 'landing-002', 'landing-003'));
...
as $$
  with exposed_sessions as (
    ...
    where ae.event_name = 'landing_visit'
      and ae.experiment_id = 'landing-003'
