...
export function asistanYanitiniSunumaDonustur(answer) {
  const metin = String(answer || "")
    .replace(/\r/g, "")
    .replace(/\s*•\s*/g, "\n• ")
    .replace(/\s*((?:Senden gereken|Yapman gereken):)\s*/gi, "\n$1 ")
    .trim();
  ...
  for (const satir of satirlar) {
    ...
    else if (/^(?:senden gereken|yapman gereken)\s*:/i.test(satir)) sonrakiAdim = temizle(satir.replace(/^(?:senden gereken|yapman gereken)\s*:/i, ""));
