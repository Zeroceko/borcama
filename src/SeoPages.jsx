...
function OdemeTakvimi() {
  const [odemeler, setOdemeler] = useState([{ id: "1", ad: "Kredi kartı", gun: "10", tutar: "7500" },{ id: "2", ad: "Kredi taksiti", gun: "18", tutar: "4200" }]);
  const toplam = odemeler.reduce((sum, item) => sum + (Number(item.tutar) || 0), 0);
  const sirali = [...odemeler].sort((a, b) => Number(a.gun) - Number(b.gun));
  ...
  return <ToolLayout title="Aylık ödeme takvimi" lead="Bu ay ödeyeceğin kart, kredi ve diğer borçları günlerine göre sırala; toplam aylık yükünü gör." path="/araclar/aylik-odeme-takvimi" schema={schema} faq={[["Bilgilerim kaydediliyor mu?","Hayır. Bu sayfadaki tutarlar yalnızca açık tarayıcı sekmesinde tutulur."],["Kalıcı takip nasıl yapılır?","Ücretsiz Borcama hesabında dönemleri ve gerçekleşen ödemeleri kaydedebilirsin."]]}><div className="seo-panel seo-wide-form"><h2>Bu ayın ödemeleri</h2><div className="seo-debts calendar">{odemeler.map((item) => <div className="seo-debt-row" key={item.id}><input aria-label="Ödeme adı" value={item.ad} onChange={(event) => guncelle(item.id,"ad",event.target.value)}/><input aria-label={`${item.ad} ödeme günü`} type="number" min="1" max="31" value={item.gun} onChange={(event) => guncelle(item.id,"gun",event.target.value)} placeholder="Gün"/><input aria-label={`${item.ad} tutarı`} type="number" value={item.tutar} onChange={(event) => guncelle(item.id,"tutar",event.target.value)} placeholder="Tutar TL"/><button aria-label={`${item.ad} sil`} onClick={() => setOdemeler((liste) => liste.filter((odeme) => odeme.id !== item.id))}><Trash2 size={16}/></button></div>)}</div><button className="seo-add" onClick={() => setOdemeler((liste) => [...liste,{id:crypto.randomUUID(),ad:`Ödeme ${liste.length+1}`,gun:"1",tutar:"0"}])}><Plus size={16}/> Ödeme ekle</button></div><div className="seo-result"><span className="seo-result-kicker">AYLIK TOPLAM</span><div className="seo-big-money"><Money value={toplam}/></div><div className="seo-calendar-list">{sirali.map((item) => <div key={item.id}><b>{item.gun || "—"}</b><span>{item.ad}</span><strong><Money value={item.tutar}/></strong></div>)}</div><Disclaimer text="Bu geçici plan yalnızca tarayıcında görünür. Ödeme talimatı oluşturmaz ve bankana veri göndermez."/></div></ToolLayout>;
}
