...
export default function Analytics() {
  ...
  return <div className="an"><style>{CSS}</style><main className="an-wrap">
    ...
    {hata?<div className="an-error">{hata}</div>:yukleniyor?<div className="an-panel an-empty">Veriler hazırlanıyor…</div>:!veri?.available?<div className="an-panel an-empty">Ölçüm altyapısı henüz veri toplamaya başlamadı.</div>:<>
      ...
      <section className="an-card"><h2>Kaynak → kayıt sonucu</h2><p>Son 90 gündeki ilk temasın kayıt ve doğrulamaya etkisi</p><div className="an-sources">{(veri.sources||[]).map(x=><div className="an-source" key={`${x.source}:${x.medium}:${x.campaign}`}><div className="an-source-head"><div><span>{x.source || "direct"}{x.medium ? ` / ${x.medium}` : ""}</span><small>{x.campaign || "Kampanya belirtilmedi"}</small></div><b>{yuzde(x.accounts_verified,x.visitors)}</b></div><div className="an-source-stats"><span>Ziyaret<b>{x.visitors || 0}</b></span><span>Kayıt ekranı<b>{x.register_views || 0}</b></span><span>Hesap<b>{x.accounts_created || 0}</b></span><span>Doğrulandı<b>{x.accounts_verified || 0}</b></span></div></div>)}</div></section></div>
      <div className="an-note">Kaynak raporu ilk temas bilgisini kullanır. Ziyaretçi ölçümü IP adresi veya e-posta saklamaz. Kullanıcı bazındaki kaynak CRM'de kayıt anındaki birinci taraf verisinden gösterilir; e-posta ve finansal bilgiler Google Analytics'e gönderilmez.</div>
    </>}
  </main></div>;
}
