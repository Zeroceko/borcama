...
export default function Analytics() {
  ...
  return <div className="an"><style>{CSS}</style><main className="an-wrap">
    ...
    {hata?<div className="an-error">{hata}</div>:yukleniyor?<div className="an-panel an-empty">Veriler hazırlanıyor…</div>:!veri?.available?<div className="an-panel an-empty">Ölçüm altyapısı henüz veri toplamaya başlamadı.</div>:<>
      ...
      <section className="an-card"><h2>Kaynak → kayıt sonucu</h2><p>Son 90 günde başlayan ölçülmüş tarayıcı oturumları</p><div className="an-sources">{(veri.sources||[]).length===0?<div className="an-empty">Bu dönem için ölçülmüş kaynak yok.</div>:(veri.sources||[]).map(x=><div className="an-source" key={`${x.source}:${x.medium}:${x.campaign}`}><div className="an-source-head"><div><span>{x.source || "direct"}{x.medium ? ` / ${x.medium}` : ""}</span><small>{x.campaign || "Kampanya belirtilmedi"}</small></div><b>{yuzde(x.accounts_created,x.visitors)}</b></div><div className="an-source-stats"><span>Ziyaret<b>{x.visitors || 0}</b></span><span>Kayıt ekranı<b>{x.register_views || 0}</b></span><span>Hesap<b>{x.accounts_created || 0}</b></span><span>Doğrulandı<b>{x.accounts_verified || 0}</b></span></div></div>)}</div></section></div>
      <section className="an-card" style={{marginTop:18}}><h2>PMax kontrol hunisi</h2><p>Yalnız ilk teması `google / cpc / tr_pmax_borcama` olan oturumlar. Reklam tıklaması değil, ölçülmüş landing ziyareti sayılır.</p><div className="an-source-stats"><span>Landing ziyareti<b>{veri.pmax?.landing_visit || 0}</b></span><span>Kayıt ekranı<b>{veri.pmax?.register_view || 0}</b></span><span>Kayıt<b>{veri.pmax?.sign_up || 0}</b></span><span>Doğrulandı<b>{veri.pmax?.email_verified || 0}</b></span><span>İlk borç/ekstre<b>{veri.pmax?.first_debt_or_statement || 0}</b></span></div></section>
      <section className="an-card" style={{marginTop:18}}><h2>LANDING-003 · Eşit teklifli tam sayfa deneyi</h2><p>Yeni landing ziyaretçileri aynı teklif ve ana mesajı kullanan iki sayfa düzeni arasında eşit ve kalıcı dağıtılır. Ana karar kayıt oranı; kalite kontrolü doğrulama ve ilk borç/ekstre oranıdır.</p><div className="an-sources">{(veri.landing_experiment||[]).length===0?<div className="an-empty">Deney henüz ziyaret almadı.</div>:(veri.landing_experiment||[]).map(x=><div className="an-source" key={x.variant}><div className="an-source-head"><div><span>{x.variant==="variant"?"B · Yeni landing":"A · Mevcut landing"}</span><small>{x.variant}</small></div><b>{yuzde(x.sign_up,x.landing_visit)}</b></div><div className="an-source-stats"><span>Ziyaret<b>{x.landing_visit||0}</b></span><span>Kayıt ekranı<b>{x.register_view||0}</b></span><span>Kayıt<b>{x.sign_up||0}</b></span><span>Doğrulandı<b>{x.email_verified||0}</b></span><span>İlk borç/ekstre<b>{x.first_debt_or_statement||0}</b></span></div></div>)}</div></section>
      <section className="an-card" style={{marginTop:18}}><h2>Mevduat aracı → Borcama hunisi</h2><p>Son 90 günde mevduat hesaplayıcısıyla başlayan anonim oturumlar. Kişisel bilgi veya hesaplanan tutar toplanmaz.</p><div className="an-source-stats"><span>Araç ziyareti<b>{veri.deposit_tool?.landing_visit||0}</b></span><span>Sonucu gördü<b>{veri.deposit_tool?.result_view||0}</b></span><span>Borcama'ya geçti<b>{veri.deposit_tool?.product_click||0}</b></span><span>Kayıt ekranı<b>{veri.deposit_tool?.register_view||0}</b></span><span>Hesap<b>{veri.deposit_tool?.sign_up||0}</b></span><span>Doğrulandı<b>{veri.deposit_tool?.email_verified||0}</b></span><span>İlk varlık<b>{veri.deposit_tool?.first_asset||0}</b></span><span>İlk borç/ekstre<b>{veri.deposit_tool?.first_debt_or_statement||0}</b></span></div><div className="an-note">14 günlük sonucu görmek için olgunlaşan kohort: <b>{veri.deposit_tool?.matured_14d||0}</b>. Yeni ziyaretçilerin aktivasyon için henüz zamanı olabilir.</div></section>
      <div className="an-note">Kaynak raporu her session_id için bu tarayıcı oturumundaki ilk teması kohort başlangıcı kabul eder; yüzde, ziyaretçiden hesap oluşturmaya geçiştir. Ölçülemeyen eski hesaplar doğrudan trafiğe eklenmez. Ziyaretçi ölçümü IP adresi veya e-posta saklamaz.</div>
    </>}
  </main></div>;
}
