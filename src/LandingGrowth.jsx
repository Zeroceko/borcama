  ...
  return <div className="lg">
    ...
    <main>
      <section className="lg-shell lg-hero">
        <div className="lg-hero-copy"><span className="lg-eyebrow">PARANIN PLANI, SENİN ELİNDE.</span><h1>{deposit ? <>Birikimini gör.<br/><em>Aylık planını<br/>tamamla.</em></> : <>Paran nereye<br/>gidiyor?<br/><em>Borcunu gör,<br/>kontrolü al.</em></>}</h1><p>{deposit ? "Nakit ve mevduatını, gelirini ve aylık ödemelerini aynı yerde takip et. Birikiminin bütün tabloda nerede durduğunu gör." : "Kart, kredi ve ek hesap borçlarını tek tabloda topla. Ödeme günlerini ve önce nereye odaklanacağını gör."}</p><div className="lg-hero-actions"><Signup href={signup}/><a className="lg-text-link" href="/demo">Örnek hesabı incele <ChevronRight size={17}/></a><a className="lg-text-link lg-mobile-how" href="#nasil-calisir">Nasıl çalışır? <ChevronRight size={17}/></a></div><div className="lg-reassurance"><span><Check size={15}/> Süresiz Ücretsiz plan</span><span><Check size={15}/> Kart bilgisi gerekmez</span></div></div>
      ...
      <section className="lg-shell lg-section lg-how" id="nasil-calisir"><div className="lg-section-head"><span className="lg-eyebrow">AZ UĞRAŞ. DAHA NET GÖR.</span><h2>Bir kayıtla başla.<br/>Tablon adım adım oluşsun.</h2><p>Her şeyi ilk gün doldurman gerekmiyor.</p></div><div className="lg-features">
        <article><span className="lg-step-number">01</span><FileText size={27}/><h3>Ekstren, anlaşılır<br/>harcamalara dönüşsün.</h3><p>Desteklenen ekstrelerde kalemler kategorilere ayrılır. Kontrol et, düzelt ve kaydet. İstersen elle başla.</p><div className="lg-mini-categories">{example.categories.map((category) => <span key={category.name}>{category.name} <b>₺{money(category.amount)}</b></span>)}</div><small>Örnek ekstre dağılımı</small></article>
