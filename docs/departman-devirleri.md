...
- Baz sürüm `v1.55.4`: `/finansal-sozluk` altında doğru Türkçe terimi kullanan, ASCII ve kalıcı slug yapılı 24 terim hazırlandı. Faiz, temerrüt, temerrüt faizi, tasfiye, kredi kartı, kredi, KMH ve bütçe kavramları; tanım, açıklama, örnek, Borcama bağlamı ve ilgili terimlerle birbirine bağlandı. Landing'in iki kolu ve SEO sayfaları Finansal Sözlük'e iç bağlantı veriyor. Her rota kendine ait statik HTML, başlık, açıklama, canonical, Article + DefinedTerm ve Breadcrumb şemasına sahip; sitemap sözlük verisinden derleme sırasında üretiliyor. Değişen alanlar `src/financialGlossary.js`, `src/SeoPages.jsx`, SEO stilleri/rotaları, landing footer'ları, prerender, sitemap, Vercel rewrite'ları, testler ve sürüm kayıtlarıdır. 194/194 otomatik test, production build, diff kontrolü, masaüstü ve 390px mobil görsel kontrol başarılıdır. Yayın adayı `v1.56.0`; sonraki dış adım canlı URL/canonical/sitemap doğrulaması ve Search Console sitemap yenilemesidir.

### 2026-09-22 · Ürün ve Mühendislik · Kalan kredi ödeme yükünün ayrıştırılması

- Baz sürüm `v1.55.0`: Kredi PDF aktarımı ve kredi satırı artık “kalan anapara”, “plana göre kalan toplam ödeme” ve “kalan faiz/masraf” tutarlarını ayrı gösteriyor. Tam ve kısmi kredi ödemeleri kalan plan toplamından düşülüyor; ödeme planındaki sıradaki satır kalan anapara projeksiyonunu ilerletiyor. Plan yüklenmeden önce uygulamaya kaydedilmiş ödemeler başlangıç değeri olarak saklandığından geçmiş ödeme ikinci kez düşülmüyor. Erken kapama tutarıyla karışmaması için kullanıcıya açık uyarı eklendi. Aynı plan özeti Asistan'ın faiz bağlamında da kullanılıyor. Değişen alanlar `src/loanPlanParser.js`, `src/loanPlanSummary.js`, `src/App.jsx`, `src/assistantContext.js`, testler ve değişiklik kaydıdır. `release:check`, 186/186 otomatik test, production build, diff kontrolü ve yerel kredi satırı görsel kontrolü başarılıdır. Gerçek kullanıcı verisi kullanılmadı; yayın sürümü `v1.55.1`dir.
...
- 2026-09-19 · Asistan Kalitesi ve Finansal Anlatım · `v1.54.5` canlı faiz yanıtında modelin ürettiği rakamlı cevap kalite kapısından geçmeyince kullanıcı genel hata görüyordu. Vergi varsayımı sayısal dayanak olarak bağlama eklendi; ayrıca faiz/maliyet sorularında model çağrısı veya doğrulaması başarısız olursa `faizMaliyetOzeti` üzerinden kısa cevap, kart/KMH aylık tahmini ve planı bilinen kredi maliyetini sunan deterministik yedek yanıt eklendi. Yedek yanıt aynı güvenlik doğrulayıcısından geçiyor. Asistan testleri 37/37, tüm testler 175/175, production build ve diff kontrolü geçti. Yayın adayı `v1.54.6`; canlı testte başarısız soru hakkı sunucu tarafından iade edildi.

- 2026-09-19 · Asistan Kalitesi ve Finansal Anlatım · Faiz sorularının “kesin hesaplanamaz” uyarısıyla rakamsız kalmaması için bağlama deterministik `faizMaliyetOzeti` eklendi. Aktif kart ve KMH için kullanıcı/TCMB/ürün referans oranından bir aylık faiz ile yüzde 30 BSMV+KKDF varsayımı; ödeme planı bilinen krediler için kalan taksit toplamı ve kalan anapara farkı ayrı hesaplanıyor. Prompt, bu iki dönemi tek kesin toplamda birleştirmeden bilinen alt toplamları mutlaka rakamla yanıtlamayı zorunlu kılıyor. Asistan testleri 37/37, tüm testler 173/173, production build ve diff kontrolü geçti. Yayın adayı `v1.54.5`; gerçek kullanıcı verisi testlere veya kayda alınmadı.

- 2026-09-19 · Asistan Kalitesi ve Finansal Anlatım · Canlı kullanıcı testinde kapanmış veya sıfırlanmış eski borçların boş faiz alanları aktif kart/KMH faizi eksikmiş gibi yorumlanıyordu. Asistan bağlamı artık faiz, taksit ve bakiye eksiklerini yalnız aktif borçlarda işaretliyor; her borç kaydına `aktifBorc` bilgisi ekleniyor ve ortak prompt kapanmış kayıtları faiz değerlendirmesinin dışında tutuyor. Asistan testleri 36/36, tüm testler 172/172, production build ve diff kontrolü geçti. Yayın adayı `v1.54.4`; gerçek kullanıcı verisi testlere veya kayda alınmadı.

- 2026-09-18 · Landing/CRO · Baz sürüm v1.54.1: `landing-bulgular.md` doğrultusunda iki kolun ücretsiz başlangıç, 30 günlük Pro hediyesi, ana hero mesajı ve kayıt hedefi eşitlendi; gizlilik ifadesi ham dosya ile hesap kaydını ayırıyor. Sayfa içindeki örnek tutarları tutarlılaştırıldı, 12px altı açıklamalar büyütüldü, mobil açıklama bağlantısı ve yüksek kontrastlı hero şeritleri eklendi. LANDING-003 yeni atama anahtarı ve ayrı rapor filtresiyle hazır; eski deney verisi silinmez. Bu tam sayfa sunum deneyidir, yalnız yerleşim testi değildir. 171/171 test, build ve diff check geçti; 390px mobil kontrolde iki kolda yatay taşma görülmedi. Kod ayrı `codex/landing-003-fixes` dalındadır, canlıya çıkmadı. Yayın sırası: deney-002 sonuç snapshot'ı → migration → analytics-event → frontend; ardından canlı atama ve huni doğrulaması. Güncel canlı huni snapshot'ı bu çalışmada alınmadı. Bulgudaki sabit faiz oranı eklenmedi: dönem toplamının faiz içerip içermediği bilinmeden ikinci kez faiz eklemek yanıltıcı olur.

- 2026-09-19 · Asistan Kalitesi ve Finansal Anlatım · Canlı eval ekranındaki “kart/KMH faiz oranları kayıtlı değil” hatasının kökü bulundu: `borcKalemleri` kart ve KMH oranlarını TCMB/ürün referansıyla çözerken `assistantContext` yalnız ham `veri.*.faiz` alanını okuyordu. Bağlam artık çözülmüş `kalemler` faizini, `faizKaynak` ve `faizTahmini` işaretleriyle kart/KMH ve diğer borçlara taşıyor; `veriEksikleri` ve `pahaliBorcVar` da aynı çözülmüş kaynağı kullanıyor. `src/assistantContext.test.js` içine regresyon eklendi. Bu depoda TCMB canlı API çağrısı bulunmadığı, oranların mevcut App içindeki referans eşiklerinden geldiği ayrıca doğrulandı. Sürüm `v1.54.3` olarak ana dala ve etikete gönderildi; 172/172 test, production build, release check ve diff kontrolü geçti. `financial-assistant` Edge Function v22 olarak etkin, canlı web paketi de çözülmüş faiz kaynaklarını içeriyor. Gerçek kullanıcı verisi kullanılmadı ve ücretli canlı model çağrısı yapılmadı.

- 2026-09-17 · Asistan Kalitesi · Başkan'ın devam sorusu isteği için son beş başarılı soru/yanıt bellekte tutularak Gemini'ye ayrı user/model turları halinde aktarılıyor. Pencere kapanınca bağlam korunuyor; Yeni konuşma, hesap değişimi ve sayfa yenileme ile temizleniyor. Sunucu yalnız sınırlı question/answer çiftlerini kabul ediyor; keyfi rol/metadata aktarılmıyor. Güncel finansal özet ve son kullanıcı düzeltmesi esas, önceki model cevabı doğrulanmış veri değil. Kısa sayısal devam yanıtları destekleniyor; geçmiş kullanıcı teklifi oran dayanağı olabilir, geçmiş modelin uydurduğu oran olamaz. İki sentetik araba/devam ve tutar düzeltme evali eklendi. Değişiklik App, istemci, ortak konuşma/prompt/doğrulama, eval/test ve süreç kayıtlarını kapsıyor; canlı deploy veya gerçek kullanıcı verisi işlemi yapılmadı.

  Doğrulama: Asistan testleri 29/29, tüm testler 155/155, production build, release check ve diff kontrolü başarılıdır. Gerçek model davranışı ücretli canlı sentetik eval çalıştırılmadığı için henüz doğrulanmadı; Edge Function ve frontend birlikte yayımlanmalıdır.

- 2026-09-17 · Asistan Kalitesi · Sentetik canlı eval koşucusu hazırlandı; Edge Function ile aynı prompt ve sunucu doğrulayıcısını kullanıyor, açık `ASSISTANT_EVAL_LIVE_CONFIRMED=true` olmadan ücretli çağrı yapmıyor ve kullanıcı/CRM kaydı oluşturmuyor. `npm run test:assistant` 23/23 başarılıdır. Canlı model çağrısı yapılmadı; referans metin testleri gerçek model başarısı olarak raporlanmayacaktır.

- 2026-09-08 · Asistan Kalitesi ve Finansal Anlatım · Baz sürüm `v1.51.1`, ikinci kalite paketi: Gemini yanıtı kullanıcıya ve CRM geçmişine geçmeden önce ortak sunucu doğrulayıcısından geçiriliyor. Zorunlu alan türleri, izinli rota, alan uzunlukları, `Kısa cevap` + 2–3 madde yapısı, 130 kelime sınırı, kesin kredi/onay/sonuç dili, banka hesabına erişim veya kullanıcı adına kayıt değiştirme iddiası ve bağlamda ya da soruda bulunmayan yüzde/faiz oranı fail-closed olarak reddediliyor; geçersiz yanıtta kota iade ediliyor. Aynı güvenlik kapısı sentetik eval rubriğiyle ortaklaştırıldı. Değişen dosyalar: `supabase/functions/_shared/financialAssistantValidation.js`, `supabase/functions/financial-assistant/index.ts`, `evals/financial-assistant/rubric.js`, `src/assistantQuality.test.js`, `CHANGELOG.md` ve bu devir kaydı. Gerçek kullanıcı verisi, canlı model çağrısı veya yayın kullanılmadı. Sonraki adım: gerçek model çıktısını yalnız sentetik vakalarla çağıran, ağ erişimi ve maliyet için açık kontrollü bir eval çalıştırıcısı hazırlanmalıdır.

- 2026-09-08 · Asistan Kalitesi ve Finansal Anlatım · Baz sürüm `v1.51.1`: Asistanın altı aylık dönem bağlamında düzenli gelir ve giderler aynı kurala bağlandı; düzenli tutarların geçmiş gerçekleşme değil tahmin olduğu modele açıkça bildirildi. Eksik kredi, kart ve KMH sayıları `0` yerine `null` olarak korunuyor, eksik alan adları profile ekleniyor ve faiz bilgisi yokken “pahalı borç yok” sonucu üretilmiyor. Varlık dağılımı mevduat, gayrimenkul ve araç gibi gerçek türleri koruyor. Değişen dosyalar: `src/assistantContext.js`, `src/assistantContext.test.js`, `supabase/functions/financial-assistant/index.ts`, `CHANGELOG.md` ve bu devir kaydı. `npm run test:assistant` 17/17, `npm test` 143/143, `npm run build`, `npm run release:check` ve `git diff --check` başarılıdır. Gerçek kullanıcı verisi kullanılmadı, canlı model çağrısı veya yayın yapılmadı. Sonraki adım: sunucu tarafında yanıt biçimi, sayı dayanağı ve güvenlik kuralları model çıktısından sonra da doğrulanmalıdır.
...
- Yapılan değişiklik: `/faq`, kendi başlık, açıklama, canonical, yapılandırılmış veri ve anlamlı ilk HTML içeriğiyle ön oluşturulur hale getirildi; Vercel yönlendirmesi bu dosyaya bağlandı. `v1.49.3` commit `c9ab91f` ile yayımlandı ve etiketlendi.
...
- Baz alınan Borcama sürümü: `v1.52.2`; Borcama Asistanı v1.46–v1.49 yetenekleriyle canlıdır.
- Yapılan değişiklik: `Siz istediniz, biz yaptık - 3 -` konulu, üç gerçek kullanıcı sorusu ve iki `Borcama'ya sor` CTA'sı taşıyan kampanya; Marketing ekranı, takipli gönderim altyapısı, UTM ve statik önizlemeyle hazırlandı. Hem büyük tanıtım rozeti hem alt düğme, oturum açıksa asistanı doğrudan açacak; oturum kapalıysa girişten sonra aynı hedefe dönecek biçimde bağlandı.
