...
- 2026-09-05 · Landing ve CRO · Baz sürüm `v1.42.8`: LANDING-001 iki aşamaya ayrıldı. Bu teslim yalnız PMax kontrol hunisini ekler; landing CTA/metni, kullanıcı ataması ve deney exposure değiştirilmedi, varyant trafiği `%0` kaldı. `20260905110000_pmax_control_funnel.sql`, `backoffice` ve Analytics ekranı ilk teması `google / cpc / tr_pmax_borcama` olan oturumlarda landing ziyareti, kayıt ekranı, kayıt, doğrulama ve ilk borç/ekstre adımını toplulaştırılmış gösterir. Reklam tıklaması huniye dahil edilmez; e-posta, kullanıcı kimliği ve finansal ayrıntı rapora taşınmaz. Canlı yayın/deploy yapılmadı. Sonraki karar: migration, `backoffice` Edge Function ve web paketi birlikte yayımlandıktan sonra kontrol verisi toplanacak; CEO ayrı kararla A/B aşamasını değerlendirecek.
